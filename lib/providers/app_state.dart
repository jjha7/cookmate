...
class AppState extends ChangeNotifier {
  ...
  void clearAll() {
    favorites.clear();
    for (final k in planner.keys) { planner[k] = []; }
    _savePrefs(); notifyListeners();
  }

  void setTheme(ThemeMode mode) {
    themeMode = mode; _savePrefs(); notifyListeners();
  }

  /// Bonus: quick suggestion, biased to favorites if any.
  Recipe? suggest({String? tag}) {
    final pool = tag == null ? all : all.where((r) => r.tags.contains(tag)).toList();
    if (pool.isEmpty) return null;
    final favPool = pool.where((r) => favorites.contains(r.id)).toList();
    final source = (favPool.isNotEmpty && Random().nextBool()) ? favPool : pool;
